"""Chunk + embed + upsert into fastvc_rag."""
...
def upsert_document(doc: DocIn, *, replace: bool = False) -> int:
    ...
    with connect() as conn, conn.cursor() as cur:
        if replace:
            cur.execute(
                "DELETE FROM fastvc_rag.documents WHERE title = %s AND source_path IS NOT DISTINCT FROM %s",
                (doc.title, doc.source_path),
            )

        cur.execute(
            """
            INSERT INTO fastvc_rag.documents (company_id, doc_type, title, source_path, metadata)
        ...
        for i, (text, vec) in enumerate(zip(chunks, vectors)):
            cur.execute(
                """
                INSERT INTO fastvc_rag.chunks (document_id, ord, text, token_count)
                VALUES (%s, %s, %s, %s)
                RETURNING id
                """,
            ...
            cur.execute(
                "INSERT INTO fastvc_rag.embeddings (chunk_id, embedding) VALUES (%s, %s::vector)",
...
def build_ann_index(lists: int = 100) -> None:
    ...
    """
    with connect() as conn, conn.cursor() as cur:
        cur.execute("SELECT count(*) FROM fastvc_rag.embeddings")
        n = cur.fetchone()[0]
        if n < 100:
        ...
        cur.execute("DROP INDEX IF EXISTS fastvc_rag.embeddings_cosine_idx")
        cur.execute(
            f"CREATE INDEX embeddings_cosine_idx ON fastvc_rag.embeddings "
