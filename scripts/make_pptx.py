"""Generate a FastVC product-tour .pptx — editable slide deck for sharing.

Mirrors the content of scripts/make_pdf.py but writes to docs/fastvc-product-tour.pptx
...
OUT = ROOT / "docs" / "fastvc-product-tour.pptx"

# 16:9 slide — 33.87 × 19.05 cm
SLIDE_W = Cm(33.87)
SLIDE_H = Cm(19.05)

# FastVC palette (parchment + forest)
...
def _blank(prs: Presentation):
    ...
    r = p.add_run()
    r.text = "FastVC · Your Venture Capital AI Agent Squad"
    r.font.size = Pt(8)
    r.font.name = "Inter"
...
def _hero_slide(prs):
    ...
    _text(s, Cm(0), Cm(6.5), SLIDE_W, Cm(3.5),
          "FastVC", size=54, bold=True, color=INK, align=PP_ALIGN.CENTER)
...
def _agenda_slide(prs):
    ...
    rows = [
        ("01", "Chat — your VC AI squad on call",
...
def _closing_slide(prs):
    ...
    _text(s, Cm(0), Cm(13.5), SLIDE_W, Cm(1),
          "hello@fastvc.fyi   ·   fastvc.fyi/contact",
...
def build() -> None:
    ...
    _content_slide(
        ...
        bullets=[
            "Filter chips: sector, and ownership (founder, family, VC, VC, carve-out).",
