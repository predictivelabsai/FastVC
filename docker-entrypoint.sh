#!/usr/bin/env sh
# Container entrypoint for FastVC.
#
...
#
# Set FASTVC_SKIP_MIGRATE=1 to bypass migration (e.g. read-only replicas).

set -eu

if [ "${FASTVC_SKIP_MIGRATE:-${PEHERO_SKIP_MIGRATE:-0}}" != "1" ]; then
