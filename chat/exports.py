...
async def export_xlsx(request: Request):
    ...
    rows = data.get("rows", [])
    title = data.get("title", "FastVC Export")
    ...
    filename = f"fastvc-{safe_title}.xlsx"
...
async def export_docx(request: Request):
    ...
    title = form.get("title", "FastVC Document")
    ...
    filename = f"fastvc-{safe_title}.docx"
...
async def generate_chart(request: Request):
    ...
    fig.update_layout(
        ...
        font=dict(family="Inter, Calibri, sans-serif", size=12, color="#141B34"),
        title=dict(font=dict(size=16, color="#3157D5")),
        plot_bgcolor="#F7F8FC",
