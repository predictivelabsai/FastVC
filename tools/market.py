"""Market / comp tools — VC transaction comps, public trading comps, sector signals."""
...
def _find_transaction_comps(**kw) -> str:
    ...
    sql = ["SELECT target_name, acquirer, sector, sub_sector, country, announce_date, "
           ...
           "FROM fastvc.transaction_comps WHERE TRUE"]
    params: list = []
    if args.slug_or_id:
    ...
    return "__ARTIFACT__" + json.dumps({
        "kind": "table",
        "title": "Comparable venture rounds",
        "subtitle": f"{len(rows2)} rounds · avg post-money / ARR {avg_rev}x",
...
find_transaction_comps = StructuredTool.from_function(
    ...
    description="Return comparable venture financings by startup, sector, sub-sector, geography and post-money range.",
...
def _find_trading_comps(**kw) -> str:
    ...
    sql = ["SELECT ticker, peer_name, sector, market_cap, ev, revenue_ltm, ebitda_ltm, "
           ...
           "FROM fastvc.trading_comps WHERE TRUE"]
    params: list = []
    cid = _resolve_cid(args.slug_or_id)
    if cid:
        sql.append("AND company_id = %s"); params.append(cid)
...
class MarketSignalsArgs(BaseModel):
    ...
    metric: Optional[str] = Field(
        default=None,
        description="arr_multiple_median | round_size_median | round_volume | fundraising_time | down_round_rate | exit_arr_multiple",
    )
...
def _fetch_market_signals(**kw) -> str:
    ...
    sql = ["SELECT sector, sub_sector, metric, value, as_of_date, source "
           "FROM fastvc.market_signals WHERE sector = %s"]
...
fetch_market_signals = StructuredTool.from_function(
    ...
    description="Fetch venture market signals such as ARR multiples, round size and volume, fundraising time, down-round rate and exit multiples.",
