...
def _abstract_contract(**kw) -> str:
    ...
    sql = ["SELECT c.*, co.name AS company_name FROM fastvc.contracts c "
           "JOIN fastvc.companies co ON co.id = c.company_id WHERE c.status = 'active'"]
    params: list = []
    if args.slug_or_id:
        try:
...
DD_CHECKLIST = [
    ...
    ("metrics", "Monthly ARR, retention, margin, burn and runway metrics"),
    ("product_gtm", "Product usage, customer references and go-to-market evidence"),
    ("tech_ddq", "Tech & security DDQ, pen-test summary"),
    ("industry", "Independent industry / commercial report"),
    ("pitch_deck", "Current investor deck with clearly dated claims"),
...
def _audit_vdr(slug_or_id: str) -> str:
    ...
    rag_docs = fetch_all(
        "SELECT doc_type, count(*) AS n FROM fastvc_rag.documents "
        "WHERE company_id = %s GROUP BY doc_type",
        (cid,),
    )
    rag_counts = {r["doc_type"]: r["n"] for r in rag_docs}
    contracts = fetch_one(
        "SELECT count(*) as n FROM fastvc.contracts WHERE company_id = %s AND status='active'",
        (cid,),
    )
    fin = fetch_one("SELECT count(*) as n FROM fastvc.financials WHERE company_id = %s", (cid,))
    cap = fetch_one("SELECT count(*) as n FROM fastvc.cap_tables WHERE company_id = %s", (cid,))
...
audit_vdr = StructuredTool.from_function(
    func=_audit_vdr,
    name="audit_vdr",
    description="Audit a company's virtual data room against a VC DD checklist; flags missing items.",
    args_schema=VdrArgs,
)
# Back-compat alias
...
def _ops_findings(slug_or_id: str, query: str = "") -> str:
    ...
    if not cid:
        return "Company not found."
    chunks = retrieve(query or "product usage pricing unit economics organization",
                      k=5, doc_types=["metrics", "product_gtm", "tech_ddq", "industry"], company_id=cid)
    if not chunks:
...
def _record_dd_finding(company_id: int, agent_slug: str, category: str,
    ...
    with connect() as conn, conn.cursor() as cur:
        cur.execute(
            "INSERT INTO fastvc.dd_findings "
...
def _list_documents(slug_or_id: str) -> str:
    ...
    if not cid:
        return "Company not found."
    rows = fetch_all(
        "SELECT doc_type, title, created_at FROM fastvc_rag.documents "
...
def _list_contracts(slug_or_id: str) -> str:
    ...
    rows = fetch_all(
        ...
        "FROM fastvc.contracts WHERE company_id = %s ORDER BY annual_value DESC NULLS LAST LIMIT 50",
        (cid,),
    )
    return json.dumps([dict(r) for r in rows], default=str)
