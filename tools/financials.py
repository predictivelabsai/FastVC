...
def _normalize_ltm(slug_or_id: str) -> str:
    ...
    co = fetch_one("SELECT name, sector, sub_sector FROM fastvc.companies WHERE id = %s", (cid,))
    rows = fetch_all(
        "SELECT month, revenue, cogs, gross_profit, opex, ebitda, adjustments, adj_ebitda, "
        "arr, gross_retention, net_retention "
        "FROM fastvc.financials WHERE company_id = %s ORDER BY month ASC",
        (cid,),
    )
    if not rows:
        annual = fetch_all(
...
def _build_lbo_model(**kw) -> str:
    ...
    ltm_rows = fetch_all(
        "SELECT revenue, adj_ebitda FROM fastvc.financials "
    ...
    with connect() as conn, conn.cursor() as cur:
        cur.execute(
            "INSERT INTO fastvc.lbo_models (company_id, name, assumptions, projections, returns) "
...
build_lbo_model = StructuredTool.from_function(
    ...
    description="Build a 5-year LBO model for a company — entry / exit multiple, revenue growth, margin expansion, capex, interest, debt paydown. Persists to fastvc.lbo_models and returns the schedule + returns.",
    args_schema=LBOArgs,
)
# Back-compat alias
...
def _size_debt_stack(**kw) -> str:
    ...
    if not cid:
        return "Company not found."
    co = fetch_one("SELECT name, ebitda_ltm, ask_multiple FROM fastvc.companies WHERE id = %s", (cid,))
    ...
    with connect() as conn, conn.cursor() as cur:
        cur.execute(
            "INSERT INTO fastvc.debt_stacks (company_id, name, tranches, total_debt, total_leverage, dscr) "
...
size_debt_stack = StructuredTool.from_function(
    ...
    description="Size an LBO capital structure (senior / unitranche + mezz + revolver) given target leverage turns and coverage ratios. Persists to fastvc.debt_stacks.",
...
def _compute_returns(**kw) -> str:
    ...
    if args.lbo_model_id:
        pf = fetch_one("SELECT * FROM fastvc.lbo_models WHERE id = %s AND company_id = %s",
                       (args.lbo_model_id, cid))
    else:
        pf = fetch_one(
            "SELECT * FROM fastvc.lbo_models WHERE company_id = %s ORDER BY id DESC LIMIT 1",
            (cid,),
        )
    if not pf:
    ...
    # MOIC bridge
    ltm_rows = fetch_all(
        "SELECT adj_ebitda FROM fastvc.financials WHERE company_id = %s "
