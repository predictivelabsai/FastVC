"""Cosine similarity retrieval over fastvc_rag with metadata filters."""
...
def retrieve(
    ...
    *,
    ...
    sql = [
        ...
        FROM fastvc_rag.chunks c
        JOIN fastvc_rag.embeddings e ON e.chunk_id = c.id
        JOIN fastvc_rag.documents d ON d.id = c.document_id
        WHERE 1=1
        """
    ]
    ...
    with connect() as conn, conn.cursor() as cur:
        ...
        if log_query:
            cur.execute(
                """
                INSERT INTO fastvc_rag.rag_queries (user_id, session_id, query, top_k, filters, latency_ms)
