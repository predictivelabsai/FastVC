-- FastVC RAG schema. Idempotent. pgvector-backed retrieval over VC VDR
-- documents (CIMs, IC memos, MSAs, QoE reports, legal diligence, ESG reports,
-- industry reports, LP side letters).

CREATE EXTENSION IF NOT EXISTS vector;
CREATE SCHEMA IF NOT EXISTS fastvc_rag;

CREATE TABLE IF NOT EXISTS fastvc_rag.documents (
    id           BIGSERIAL PRIMARY KEY,
    company_id   BIGINT,              -- soft reference to fastvc.companies(id)
...
CREATE INDEX IF NOT EXISTS documents_company_idx ON fastvc_rag.documents(company_id);
CREATE INDEX IF NOT EXISTS documents_type_idx    ON fastvc_rag.documents(doc_type);

CREATE TABLE IF NOT EXISTS fastvc_rag.chunks (
    id           BIGSERIAL PRIMARY KEY,
    document_id  BIGINT NOT NULL REFERENCES fastvc_rag.documents(id) ON DELETE CASCADE,
...
CREATE INDEX IF NOT EXISTS chunks_document_idx ON fastvc_rag.chunks(document_id);
...
CREATE TABLE IF NOT EXISTS fastvc_rag.embeddings (
    chunk_id   BIGINT PRIMARY KEY REFERENCES fastvc_rag.chunks(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS fastvc_rag.rag_queries (
    id          BIGSERIAL PRIMARY KEY,
    user_id     BIGINT,
    session_id  BIGINT,
    query       TEXT NOT NULL,
