...
fastvc.companies (portfolio companies / deal targets).
"""
...
search_companies = StructuredTool.from_function(
    ...
    description="Search the FastVC company catalog (pipeline + portfolio). Filter by hq_city, hq_state, country, sector, deal_stage, ownership, or a free-text query.",
...
def _get_company(slug_or_id: str) -> str:
    try:
        cid = int(slug_or_id)
        row = fetch_one("SELECT * FROM fastvc.companies WHERE id = %s", (cid,))
    except (TypeError, ValueError):
        row = fetch_one("SELECT * FROM fastvc.companies WHERE slug = %s", (slug_or_id,))
    if not row:
        return "Not found."
